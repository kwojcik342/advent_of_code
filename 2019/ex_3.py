...
def get_coor_list(in_list):
    ...
    for i in in_list:
        ...
        if direction == "R":
            for j in range(it):
                cur_coor[0] += 1
                res_list.append(tuple(cur_coor))
        elif direction == "L":
            for j in range(it):
                cur_coor[0] -= 1
                res_list.append(tuple(cur_coor))
...
def man_dist(in_point):
    return abs(in_point[0]) + abs(in_point[1])
...
if __name__ == "__main__":
    ...
    l_cable_1_xy = get_coor_list(l_cable_1)
    l_cable_2_xy = get_coor_list(l_cable_2)

    l_join_xy = list_comp(l_cable_1_xy, l_cable_2_xy)
    l_distances = []
    for p in l_join_xy:
        l_distances.append(man_dist(p))
