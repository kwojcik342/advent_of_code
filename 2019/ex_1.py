...
def calc_fuel_req(in_m_mass):
    req_fuel = floor(in_m_mass / 3) - 2
    if req_fuel > 0:
        req_fuel += calc_fuel_req(req_fuel)
        return req_fuel
    else:
        return 0
...
if __name__ == "__main__":
    ...
    with open('ex_1.txt', "r") as f:
        for line in f.readlines():
            module_fuel_req = calc_fuel_req(int(line))
            # print(module_fuel_req)
            total_fuel_req += module_fuel_req
            # print(total_fuel_req)

    # print(calc_fuel_req(3))
    print(total_fuel_req)